          ...
          [
            ...
            try {
              ...
              if (element.getAttribute(attribute).startsWith("/")) {
                url = new URL(((!element.getAttribute(attribute).startsWith("http://") && !element.getAttribute(attribute).startsWith("https://")) ? (new URL(req.originalUrl.substring(1)).protocol + "//" + new URL(req.originalUrl.substring(1)).hostname + ((new URL(req.originalUrl.substring(1)).port) ? new URL(req.originalUrl.substring(1)).port : "") + ((!element.getAttribute(attribute).startsWith("/")) ? new URL(req.originalUrl.substring(1)).pathname : "/")) : ((element.getAttribute(attribute).startsWith("//")) ? new URL(req.originalUrl.substring(1)).protocol : "")) + element.getAttribute(attribute), req.protocol + "://" + req.get("host"));
              } else {
                url = new URL(value, req.protocol + "://" + req.get("host"));
              }
